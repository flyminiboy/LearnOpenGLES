...
class TriangleRender(surfaceView: TriangleSurfaceView) : GLSurfaceView.Renderer {
    ...
    val vbos = intArrayOf(1)
    val vbosBuffer = ByteBuffer.allocateDirect(vbos.size * 4)
        .order(ByteOrder.nativeOrder())
        .asIntBuffer()

    val vaos = intArrayOf(1)
    val vaosBuffer = ByteBuffer.allocateDirect(vaos.size * 4)
        .order(ByteOrder.nativeOrder())
        .asIntBuffer()
...
//        GLES30.glBindVertexArray(0)
    ...
    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        ...
        // 设置窗口的维度
        // glViewport函数前两个参数控制窗口左下角 ?? 的位置。第三个和第四个参数控制渲染窗口的宽度和高度（像素）
